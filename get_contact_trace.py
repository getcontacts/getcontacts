...
def main(argv=None):
    ...
    labels = parse_labels(args.labels, args.interactions)
    contacts, num_frames = parse_contacts(args.input_contacts, itypes)

    # Filter contacts and generate trace
    contact_frames = filter_contacts(contacts, interaction_patterns)
...
def parse_labels(labels, interactions):
    if labels is not None:
        if len(labels) != len(interactions):
            sys.stderr.write("Error: Only specified %d labels but there are %d interaction patterns\n" %
                             (len(labels), len(interactions)))
            sys.exit(-1)
        return labels

    return [i.replace(" ", " - ") for i in interactions]
...
def filter_contacts(contacts, interaction_patterns):
    ret = []
