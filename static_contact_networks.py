...
Command was:
python static_contact_networks.py --help

usage: python static_contact_networks.py [--help] 
					  [--topology TOPOLOGY]
...
python dynamic_contact_networks.py --topology TOP.pdb --output_dir OUTPUT_DIR --solv IP3 --sele "chain A and resid 100 to 160" --ligand EJ4 --itype -sb -hb -lhb

Pi-cation, pi-stacking, and vanderwaals contacts in the entire protein:
python dynamic_contact_networks.py --topology TOP.psf --output_dir OUTPUT_DIR --itype -pc -ps -vdw

Salt bridges and hydrogen bonds in the entire protein with modified distance cutoffs:
python dynamic_contact_networks.py --topology TOP.mae --output_dir OUTPUT_DIR --sb_cutoff_dist 5.0 --hbond_cutoff_dist 4.5 --itype -sb -hb
"""
...
def process_main_args(args):
	...
	return topology, output_dir, ligand, solv, sele
...
def main():
	...
	TOP, OUTPUT_DIR, ligand, solv, sele = process_main_args(args)
